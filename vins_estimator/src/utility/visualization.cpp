...
ros::Subscriber after_filter_sub; 
double nokov[3];
bool get_nokov=false;
Eigen::Vector3d drift_offset(0,0,0);
...
void registerPub(ros::NodeHandle &n)
{
    ...
    after_filter_sub=n.subscribe("/after_filter",5,afterfiltercbck);

    cameraposevisual.setScale(0.1);
    cameraposevisual.setLineWidth(0.01);
}
void afterfiltercbck(const std_msgs::Float32MultiArray &msg)
{
    get_nokov=true;
    nokov[0]=msg.data[0];
    nokov[1]=msg.data[1];
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, double t)
{
    ...
    // odometry.pose.pose.position.x = P.x()+X_SWARM_OFFSET;
    // odometry.pose.pose.position.y = P.y()+Y_SWARM_OFFSET;



    // cal offset lpf
    double coefficient = 0.05;
    Eigen::Vector3d nokov_position;
    nokov_position<<nokov[0],nokov[1],nokov[2];

	// ROS_INFO_STREAM("self_pose"<<P<<"drift_offset"<<drift_offset);
    odometry.pose.pose.position.x = P.x()-drift_offset.x()+X_SWARM_OFFSET;
    odometry.pose.pose.position.y = P.y()-drift_offset.y()+Y_SWARM_OFFSET;
    odometry.pose.pose.position.z = P.z()-drift_offset.z();


    // std::cout<<"vins x: "<<P.x()<<" y:"<<P.y()<<std::endl;
    if (get_nokov)
    {
	    drift_offset = coefficient * (P - nokov_position) + (1 - coefficient) * drift_offset;
        // odometry.pose.pose.position.x = nokov[0]+X_SWARM_OFFSET;
        // odometry.pose.pose.position.y = nokov[1]+Y_SWARM_OFFSET;
        // odometry.pose.pose.position.z = nokov[2];
        ROS_INFO_STREAM("gap x: "<<nokov[0]-odometry.pose.pose.position.x<<"y:"<<nokov[1]-odometry.pose.pose.position.y<<"z:"<<nokov[2]- odometry.pose.pose.position.z);
        get_nokov=false;
    }
